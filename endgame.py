...
def speak(text):
    ...
    engine.setProperty('volume', 1.0)  #CHECKED
    engine.say(text)
    engine.runAndWait()
    #-----------------------------------------------------
# shutdown laptop
...
def shutdown_vasuki():
    ...
    os.system("shutdown /s /t 7")  # Windows: Shutdown in 5 seconds
    sys.exit()  # Exit Vasuki
#-----------------------------------------------------------------------------
#setting alarm
def set_alarm(hour, minute):
    ...
    while True:
        ...
        if current_time.tm_hour == hour and current_time.tm_min == minute:
            pygame.mixer.init()
            pygame.mixer.music.load(r"C:\PROJECT\VASUKI\ADVANCED\all.mp3")
            pygame.mixer.music.play()   #CHECKED
...
# == Recognizer object ==
recognizer = sr.Recognizer()
newsapi="b355058d3d8844b5bd958013023f5824"
#gemini start
genai.configure(api_key="AIzaSyBSMGLK9ifCqHBr2tGdfgYzc1D1b2bl1wE") 
def aiProcess(command):
    """Get AI response from Google Gemini"""           #CHECKED
...
def search_youtube(song):
    """Search song on YouTube and return first video link using Selenium"""
    query = song.replace(" ", "+")
    url = f"https://www.youtube.com/results?search_query={query}&autoplay=1"
#-----------------------------------------------------------------------
    ...
    try:
        ...
        video_link = "https://www.youtube.com" + video_element.get_attribute("href")
    except:
        video_link = None  # No video found
...
def processcommand(c):
     ...
     elif "shutdown vasuki" in c.lower():
        shutdown_vasuki()
     elif "on wifi" in c.lower():
         wifi_on() 
     elif "off wifi" in c.lower():
         wifi_off()
